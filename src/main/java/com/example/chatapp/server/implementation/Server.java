package com.example.chatapp.server.implementation;
...
public class Server implements ChatService, UserService, SubscribeService, LogService {
    ...
    public ChatMsg sendGroupMessage(String sender, String groupName, String message) throws RemoteException {
        try (Session session = sessionFactory.openSession()) {
            ...
            // Notify only members of this specific group
    ...
    public void notifyChatStarted(String chatName, Date startTime) throws RemoteException {
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            // Get all subscribers of the admin
            List<ChatUser> subscribers = getSubscribers(admin.getUsername());

            // Notify all online subscribers
            for (ChatUser subscriber : subscribers) {
                ChatObserver observer = observers.get(subscriber.getUsername());
                if (observer != null) {
                    try {
                        observer.chatStarted(chatName, startTime);
                    } catch (RemoteException e) {
                        // Remove the observer if it's no longer reachable
                        observers.remove(subscriber.getUsername());
    ...
    public void notifyUserJoined(String chatName, String username, String nickName, Date joinTime) throws RemoteException {
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            // Notify all online users in the chat
            for (ChatObserver observer : observers.values()) {
                try {
    ...
    public void notifyChatStopped(String chatName, Date stopTime) throws RemoteException {
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            String filePath = saveChatLogs(chatName, chatId);
    ...
    public String saveChatLogs(String chatName, String chatId) throws RemoteException {
        System.out.println("Saving chat logs for: " + chatName + " with ID: " + chatId);

        try (Session session = sessionFactory.openSession()) {
            // Create directory if it doesn't exist
            String dirPath = System.getProperty("user.home") + File.separator + "chatLogs";
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(file))) {
                // Write chat start
                writer.println("Chat started at: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                ...
                // Write chat stop
                writer.println("Chat stopped at: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
            }

            System.out.println("Chat logs saved to: " + filePath);
            return filePath;
        } catch (Exception e) {
            System.err.println("Error saving chat logs: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
